...
function UpdateTribeBannerConversionBar(barbarianTribeEntry : table)
	...
	if(not barbarianTribeEntry.BannerInstance.ConversionBar:IsHidden())then
		...
		local iPointsToConvert : number = pBarbarianManager:GetTribeConversionPointsRequired(tribeIndex); -- Earlier
		local iRealPointsToConvert = iPointsToConvert - 10;
		if(iRealPointsToConvert <= 0)then iRealPointsToConvert = 1; end
		conversionPercent = conversionPercent * iPointsToConvert / iRealPointsToConvert; -- Fix Percentage off by 10 points
		...
		-- Conversion tip given only in chunky increments as a hint -- NERE: Now exact values are provided.
		--local iPointsToConvert : number = pBarbarianManager:GetTribeConversionPointsRequired(tribeIndex); -- Earlier
		local iPointsRemaining = iPointsToConvert - 10 - iCurrentPoints; -- Convert 10 points early
		...
		if (iPointsRemaining <= 0) then -- Convert to Free City
			local tribeType : number = pBarbarianManager:GetTribeNameType(tribeIndex);
			local pBarbTribe : table = GameInfo.BarbarianTribeNames[tribeType];
			local tribeName : string = pBarbTribe.TribeDisplayName;
			ExposedMembers.BarbarianFreeCities.InjectFreeCity(barbarianTribeEntry.Plot, tribeName);
		end
	end
end
